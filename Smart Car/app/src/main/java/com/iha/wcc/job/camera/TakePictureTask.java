...
public class TakePictureTask extends AsyncTask<String, Void, Bitmap> {
    ...
    protected void onPostExecute(Bitmap result) {
        ...
        if(filename != null){
            Toast.makeText(this.context, "\u56fe\u50cf\u5df2\u7ecf\u4fdd\u5b58! (" +filename+")", Toast.LENGTH_SHORT).show();
        }else{
            Toast.makeText(this.context, "\u56fe\u50cf\u4fdd\u5b58\u5931\u8d25!", Toast.LENGTH_LONG).show();
        }
    }
    ...
    private String storeImage(Bitmap imageData, String filename) {
        ...
        try {
            ...
            Toast.makeText(this.context, "\u65e0\u6cd5\u62cd\u7167\u3002 " + e.getMessage(), Toast.LENGTH_LONG).show();
            return null;
        } catch (IOException e) {
            Log.w(TAG, "Error saving image file: " + e.getMessage());
            Toast.makeText(this.context, "\u65e0\u6cd5\u62cd\u7167\u3002 " + e.getMessage(), Toast.LENGTH_LONG).show();
